...
void sim_router_template::receive_flit(long a, long b, flit_template & c)
{
    //added by Swain
	if (flit_trace) {
		vector<add_type> path;
		map<long, vector<add_type> >::iterator iter;
		if ((iter = flit_paths.find(c.flit_id()))
			== flit_paths.end()) {
			path = std::vector<add_type>();
			path.push_back(c.sor_addr());
		} else {
			path = iter->second;
		}
		path.push_back(address_);
		flit_paths[c.flit_id()] = path;
	}
